...
module.exports = async (client, message, args, type, channel, guild) => {
  ...
  if (method[0] === "play")
    ...
    return message.channel.send(new MessageEmbed()
      ...
      .setTitle("❌ Error | No valid search Term? ... Please Contact: `sagirinozomi`")
    );
}
...
const search = args.join(" ");
  ...
  if(message.guild.me.permissionsIn(message.channel).has("EMBED_LINKS")){
    ...
    .setAuthor(message.author.username, message.author.displayAvatarURL({dynamic: true}), "https://rythm.fm/")
    .setColor(ee.color)
    ...
    message.channel.awaitMessages(m => m.author.id == message.author.id, {
      ...
      if (index < 0 || index > max - 1) return waitforanswer();
        ...
        let playembed = new Discord.MessageEmbed()
          .setAuthor(`Added to queue`, message.author.displayAvatarURL({dynamic: true}), track.uri)
          .setURL(track.uri)
...
async function play(client, message, args, type) {
  const search = args.join(" ");
    ...
    function song_() {
      ...
      if(res.tracks[0].duration > 10 * 60 * 60 * 1000){
        return message.channel.send(`**:x: Cannot play a song that's longer than 10 hours**`)
     }
      ...
      //otherwise
          ...
          //create info msg embed
          let playembed = new Discord.MessageEmbed()
            .setAuthor(`Added to queue`, message.author.displayAvatarURL({dynamic: true}), res.tracks[0].uri)
    ...
    function playlist_() {
      ...
      for(const track of res.tracks)
        if(track.duration > 10 * 60 * 60 * 1000){
          return message.channel.send(`**:x: Cannot play a song that's longer than 10 hours --> playlist skipped!**`)
        }
      let player;
      player = client.manager.create({
        guild: message.guild.id,
      ...
      var time = 0;
        let playlistembed = new Discord.MessageEmbed()
          ...
          .setAuthor(`Playlist added to Queue`, message.author.displayAvatarURL({dynamic:true}), res.playlist.uri)
