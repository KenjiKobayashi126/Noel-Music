...
module.exports = async (client, message) => {
    ...
    let prefix = await client.settings.get(`prefix_${message.guild.id}`) || config.prefix;//await client.data.get(`prefix_${message.guild.id}`) || config.prefix;
    ...
    const args = message.content.slice(matchedPrefix.length).trim().replace(/[<>]/g, "").split(/ +/);
